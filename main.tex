...
% \usepackage[utf8]{inputenc} % Não é necessário se você estiver utilizando um compilador LaTeX baseado já em UTF-8, como o XeLaTeX
...
\usepackage{amsmath}
\usepackage{amssymb} % Pacote responsável por adicionar símbolos matemáticos. Símbolos, por exemplo, também necessários para o correto referenciamento de leis e decretos 
...
% É necessário que o pacote do índice fique após o pacote hyperref, por causa que ele afeta a identificação dos comandos \see e \seealso como próximos caracteres, para a impressão de vírgula ou não. Por um lado, sacrifica-se a praticidade dos links (o que seria muito bom), por outro, não seguiria corretamente o padrão NBR 6034 (que nem mesmo a ABNT segue exatamente como padroniza, como está presente no índice da NBR 6023:2018)
% Ob.: O pacote imakeidx é um tanto limitado em determinados recursos, então, o usuário está limitado a citar o número da página para os itens dos índices. Além disso, ele não sabe ordenar corretamente quando existem caracteres "especiais", como letras acentuadas, aí tem todo um ajuste para fazer, que pode ser visto nos exemplos dados.
% Pacote responsável por permitir criar o índice. 
...
\usepackage{lipsum} % Pacote meramente cosmético, para testes. Remova ele quando for utilizar o modelo e não tiver mais nenhum chamado para o comando \lipsum, mas só se quiser. Mal esse pacote não deve fazer, mas quanto menos pacotes, menos dor de cabeça.
